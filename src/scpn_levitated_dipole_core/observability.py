...
# SCPN Levitated Dipole Core — diagnostic and clock semantics
...
identities for the levitated-dipole configuration this repository owns,
...
from scpn_levitated_dipole_core.configuration import ConsistencyFinding
...
INTERCHANGE_BAND_HZ: Final = (1.0e0, 1.0e4)
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-LEVITATED-DIPOLE-CORE"
                )
...
class DiagnosticChannelPlan:
    ...
    timing_uncertainty_s
        ...
        candidate is applicable to SCPN-LEVITATED-DIPOLE-CORE.
    acquisition_start_s
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-LEVITATED-DIPOLE-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented dipole diagnostic practice;
            ...
            if channel.candidate_id == "open.resolved_interchange_mode" and not (
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            "declared interchange/entropy-mode band "
                            f"{channel.max_signal_frequency_hz:.1f} "
                            "Hz is outside the dipole Hz-to-kHz "
                            "instability scale "
                            "(Garnier et al. 2006)"
                        ),
                    )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "1 MHz-200 GHz used on dipole experiments "
                            "(Post 1987)"
                        ),
                    )
                )
                ...
                findings.append(
                    ConsistencyFinding(
                        ...
                        message=(
                            ...
                            "longer than the levitated-dipole shot scale of up "
                            "to ~10 s (Garnier et al. 2006)"
                        ),
                    )
                )
