...
designated owner of device-level truth for the `levitated_dipole`
configuration of the SCPN Phase Orchestrator reactor registry (closed
dipole field).
...
serialisation, and a data-only SPO registry pin (`src/scpn_levitated_dipole_core/`,
...
This repository owns, for the levitated-dipole device family:

- the device boundary: plant and experiment truth, lifecycle, and
  configuration policy for devices confining plasma in the closed-field
  dipole field of a magnetically levitated superconducting internal coil,
  where compressibility-driven turbulence can pinch particles inward and
  sustain centrally peaked profiles without driven current;
- levitation-system semantics as device truth: coil cooldown, charging,
  levitation control envelope, and catcher/failure boundary declarations;
- diagnostic semantics, reference frames (dipole flux coordinates), and
  clock identity declarations;
...
- **Magnetic mirrors**: `SCPN-MIRROR-CORE`. **Magnetic cusp**:
  `SCPN-MAGNETIC-CUSP-CORE`.
- **Toroidal external-coil systems** (tokamaks and the other closed-field
  device cores): `SCPN-TOKAMAK-CORE` and its neighbours.
...
artefact, and no parameter set describes or validates any real machine. Coil-technology and fuel-cycle choices are
