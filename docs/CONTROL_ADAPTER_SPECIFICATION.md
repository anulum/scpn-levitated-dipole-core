...
SCPN Levitated Dipole Core — CONTROL adapter specification
-->

# CONTROL adapter specification

**Adapter identifier:** `scpn-levitated-dipole-core.control-adapter`  
...
levitated-dipole adapter would supply device truth to the SCPN control
...
4. Independent machine protection retains the final veto on every plant
   ...
   protection — in particular, the levitation-safety and coil-catcher
   chain is machine-protection territory that this contract only
   describes.
...
- channel identifier and physical quantity with SI unit — including
  levitation-state channels (coil position and attitude, levitation-coil
  current, cryogenic temperatures) and plasma channels (density profiles,
  edge probes) with exact definitions declared;
- coordinate convention (dipole flux coordinate, flux-tube-volume label,
  or laboratory frame) and spatial layout (scalar, chord, or profile);
- sampling clock identity and timestamp semantics (slow cryogenic and
  fast plasma time bases declared separately);
...
- lifecycle phase (`cooldown`, `charging`, `lift`, `levitated_operation`,
  `de_levitation`, `recool`, `terminated`) with monotonic transition
  semantics and device-truth event records for levitation-control
  excursions and catcher engagements;
- configuration identity: the `levitated_dipole` registry configuration,
  the coil-system revision, and the configuration policy revision that
  produced the run;
...
For each actuator class (levitation-coil current control, charging
circuit, radio-frequency heating systems, gas fuelling, auxiliary shaping
coils):
...
- machine-readable operational envelope (coil current and temperature
  margins, levitation-position bounds, density limits with applicability
  domains);
- declared device-level hazard semantics for quench precursors,
  levitation excursions, and forced de-levitation;
...
above through CONTROL's real admission path: nominal levitated runs,
de-levitation sequences, boundary-of-envelope cases, invalid-payload
