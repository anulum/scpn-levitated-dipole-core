...
# SCPN Levitated Dipole Core — repository-level truth tests
...
REQUIRED_PATHS = (
    ...
    "src/scpn_levitated_dipole_core/__init__.py",
    "src/scpn_levitated_dipole_core/configuration.py",
    "src/scpn_levitated_dipole_core/errors.py",
    "src/scpn_levitated_dipole_core/observability.py",
...
def test_manifest_declares_exact_configuration_assignment() -> None:
    ...
    assert manifest["project"] == "SCPN-LEVITATED-DIPOLE-CORE"
...
def test_package_agrees_with_manifest_truth() -> None:
    ...
    from scpn_levitated_dipole_core import OWNED_CONFIGURATIONS, RegistryBinding
...
def test_typed_package_marker_exists() -> None:
    ...
    assert (REPO / "src" / "scpn_levitated_dipole_core" / "py.typed").is_file()
