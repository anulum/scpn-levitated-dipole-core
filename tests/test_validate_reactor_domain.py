...
# SCPN Levitated Dipole Core — reactor-domain validator tests
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"excluded_domains": None}, "excluded_domains:"),
...
def test_map_cross_check_rejects_assignment_and_pin_drift(tmp_path: Path) -> None:
    ...
    map_path.write_text(
        json.dumps(
            {
                "planned_repositories": ["SCPN-LEVITATED-DIPOLE-CORE"],
                "existing_repositories": [],
                "configuration_assignments": {
                    "conventional_tokamak": "SCPN-LEVITATED-DIPOLE-CORE"
                },
...
def _rf_kl_manifest_with_pin() -> dict[str, Any]:
    ...
    manifest["excluded_domains"] = [dict(KERNEL_EXCLUSION), *excluded]
    return manifest
...
def test_rf_kl_manifest_may_omit_both_optional_pin_and_owner_exclusion(
    tmp_path: Path,
) -> None:
    """The portable validator keeps non-consumers valid and unclaimed."""
    manifest = _rf_kl_manifest_with_pin()
    del manifest["kernel_library"]
...
def test_rf_kl_pending_registry_release_is_an_exact_admissible_pin(
    tmp_path: Path,
) -> None:
    """A prepared registry release may be consumed before the base pin moves."""
    manifest = _rf_kl_manifest_with_pin()
    ...
    map_path = tmp_path / "pending-map.json"
    map_path.write_text(json.dumps(machine_map), encoding="utf-8")
    assert (
        validate_manifest(write_manifest_with_evidence(tmp_path, manifest), map_path)
        == []
    )
