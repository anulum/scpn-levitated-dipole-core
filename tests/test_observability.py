...
# SCPN Levitated Dipole Core — diagnostic and clock semantics tests
...
from scpn_levitated_dipole_core.errors import DiagnosticPlanError
...
NONCYCLIC_BINDINGS = {
    ...
    "coordinate_frame": "frm_dipole_flux",
    "provenance": "synthetic fixture",
    "quality": "synthetic quality flags",
...
REFERENCE_FRAMES = (
    ReferenceFrame(
        identifier="frm_dipole_flux",
        kind=FrameKind.FLUX_SURFACE,
        description="dipole flux-shell label frame",
    ),
    ReferenceFrame(
        identifier="frm_field_line",
        kind=FrameKind.FIELD_LINE,
        description="field-line following frame around the floating coil",
    ),
    ReferenceFrame(
        identifier="frm_machine",
        kind=FrameKind.MACHINE_CYLINDRICAL,
        description="vacuum-vessel cylindrical frame",
    ),
)
CLOCK_RELATIONS = (
    ClockRelation(
...
REFERENCE_TRANSFORMATIONS: tuple[FrameTransformation, ...] = (
    FrameTransformation(
        source_identifier="frm_dipole_flux",
...
def channel_derived() -> DiagnosticChannelPlan:
    """Build the synthetic interchange-mode probe-array channel."""
    return DiagnosticChannelPlan(
        identifier="ch_interchange_probe_array",
        candidate_id="open.resolved_interchange_mode",
        carrier=SemanticCarrier.COMPLEX_MODE,
        clock_identifier="clk_facility",
        sample_rate_hz=1.0e05,
        max_signal_frequency_hz=1.0e03,
        timing_uncertainty_s=None,
        acquisition_start_s=0.0,
        acquisition_duration_s=5.0,
        element_count=16,
        evidence_bindings=dict(DERIVED_BINDINGS),
        signals=SIGNALS_CH_INTERCHANGE_PROBE_ARRAY,
        synthetic=True,
    )
...
def synthetic_plan() -> DiagnosticPlan:
    ...
    return DiagnosticPlan(
        identifier="dipole_reference_plan",
...
def test_candidate_rejects_malformed_identifier() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="candidate_id"):
        CandidateProfile(
            candidate_id="Bad-Id",
            phenomenon="x",
            configurations=("levitated_dipole",),
...
def test_candidate_rejects_unsorted_configurations() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        CandidateProfile(
            candidate_id="a.b",
            phenomenon="x",
            configurations=("levitated_dipole", "levitated_dipole"),
            observability_class=ObservabilityClass.NONCYCLIC_FEATURE,
        )
...
def test_channel_rejects_unsorted_or_duplicate_signals() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(signals=tuple(reversed(SIGNALS_CH_INTERCHANGE_PROBE_ARRAY)))
    with pytest.raises(DiagnosticPlanError, match="unique and sorted"):
        _derived(
            signals=(
                *SIGNALS_CH_INTERCHANGE_PROBE_ARRAY,
                SIGNALS_CH_INTERCHANGE_PROBE_ARRAY[-1],
            )
        )
...
def test_non_event_channel_rejects_timing_marker() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="only event-relative"):
        _derived(signals=(*SIGNALS_CH_INTERCHANGE_PROBE_ARRAY, marker))
...
def test_transformation_rejects_self_mapping() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="to itself"):
        _transformation(target_identifier="frm_dipole_flux")
    ...
    with pytest.raises(DiagnosticPlanError, match="duplicate transformation pair"):
        _plan_with(
            frame_transformations=(
                _transformation(),
                _transformation(
                    source_identifier="frm_machine", target_identifier="frm_dipole_flux"
                ),
            )
        )
...
def test_report_flags_cyclic_array_without_amplitude_signal() -> None:
    ...
    channel = _derived(
        signals=tuple(
            signal
            for signal in SIGNALS_CH_INTERCHANGE_PROBE_ARRAY
...
def test_channel_rejects_nyquist_violation() -> None:
    ...
    with pytest.raises(DiagnosticPlanError, match="Nyquist"):
        _derived(sample_rate_hz=1.0e03)
...
def test_plan_accepts_explicit_deferral() -> None:
    ...
    plan = DiagnosticPlan(
        identifier="dipole_partial_plan",
        binding=CATALOGUE_BINDING,
...
def test_report_flags_band_outside_typical_range() -> None:
    ...
    findings = plan.consistency_report()
    assert len(findings) == 1
    assert "Garnier" in findings[0].message
...
def test_plan_rejects_unknown_frame_reference() -> None:
    ...
    plan = synthetic_plan()
    kept = tuple(
        frame for frame in plan.frames if frame.identifier != "frm_dipole_flux"
    )
