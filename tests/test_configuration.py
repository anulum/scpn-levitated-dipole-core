...
# SCPN Levitated Dipole Core — device configuration container tests
...
def synthetic_configuration(
    identifier: str = "levitated_dipole",
    field_gradient_t_per_m: float = 0.005,
) -> DeviceConfiguration:
    ...
    return DeviceConfiguration(
        ...
        levitation=LevitationEnvironment(field_gradient_t_per_m=field_gradient_t_per_m),
        registry=REGISTRY,
    )
...
def test_owned_identifier_constructs() -> None:
    """The owned identifier constructs with a levitatable coil."""
    assert synthetic_configuration().identifier == "levitated_dipole"
...
def test_levitation_force_formula() -> None:
    """The levitation force follows ``F = m dB/dz`` exactly."""
    configuration = synthetic_configuration()
    expected = configuration.coil.magnetic_moment_a_m2() * 0.005
    assert configuration.levitation_force_n() == pytest.approx(expected)


def test_consistency_report_clean_and_finding() -> None:
    """The report is empty when the force covers the weight."""
    assert synthetic_configuration().consistency_report() == ()
    weak = synthetic_configuration(field_gradient_t_per_m=0.000001)
    findings = weak.consistency_report()
    assert len(findings) == 1
    assert "below the coil weight" in findings[0].message
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: {**r, "levitation": []}, "levitation: must be an object"),
...
def test_from_record_field_type_violations() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["coil"]["coil_radius_m"] = "big"
    with pytest.raises(DeviceConfigurationError, match="coil_radius_m: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
    record["coil"]["superconducting"] = "yes"
    with pytest.raises(DeviceConfigurationError, match="superconducting: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
    record["coil"]["coil_mass_kg"] = True
    with pytest.raises(DeviceConfigurationError, match="coil_mass_kg: must be"):
        configuration_from_record(record)
    record = synthetic_configuration().to_record()
...
def test_from_bytes_rejects_invalid_documents() -> None:
    ...
    record = synthetic_configuration().to_record()
    text = json.dumps(record).replace("1.0,", "NaN,", 1)
...
def test_integer_accepted_where_number_expected() -> None:
    ...
    record = synthetic_configuration().to_record()
    record["coil"]["coil_mass_kg"] = 500
    restored = configuration_from_record(record)
    assert restored.coil.coil_mass_kg == 500.0
