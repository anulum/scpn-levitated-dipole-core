...
SCPN Levitated Dipole Core — VALIDATION
-->
...
- Validated frozen parameter objects (`FloatingCoil`,
  `LevitationEnvironment`, `DeviceConfiguration`) rejecting non-finite
  values, non-positive extents, and a non-superconducting floating coil
  (the hard levitation invariant; LDX-class design, Garnier et al.,
  Fusion Eng. Des. 81 (2006) 2371) — every rejection branch is tested.
- Derived quantities from standard magnetostatics: the coil magnetic
  moment `m = pi R^2 I` and the axial dipole force `F = m dB/dz`.
- An advisory consistency finding, reported and never clamped: a
  levitation force below the coil weight `M g`.
...
- The estimates are advisory force-balance checks, not equilibrium,
  confinement, or stability results; no benchmark, dataset, solver,
...
- A reference plan mirroring canonical practice with synthetic
  declarations: flux loops, interferometer, drive reference, interchange probe array, synthetic oscillator, each bound to its clock domain.
- Documented advisory band checks with their sources stated in the
  code: dipole interchange/entropy-mode activity in the Hz-to-kHz scale and ICRF-to-ECRH drive frequencies (Garnier et al. 2006); findings are reported, never clamped.
- Canonical serialisation (sorted keys, NaN/infinity rejected on both
  emit and parse), SHA-256 digest identity, and a strict round-trip
  parser that refuses unknown fields.
